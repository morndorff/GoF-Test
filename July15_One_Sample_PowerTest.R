...
for (i in 1:num.dist.test) {
    names(liPVals)[i] <- paste(dist[[i]], "(", paste(param[[i]], collapse = ","), ")", 
        sep = "")
    for (j in 1:num.samps) {
...
for (i in 1:num.dist.test) {
    names(liResults)[i] <- paste(dist[[i]], "(", paste(param[[i]], collapse = ","), 
        ")", sep = "")
    for (j in 1:num.samps) {
...
for (i in 1:num.dist.test) {
    ...
    names(liTables)[i] <- paste(dist[[i]], "(", paste(param[[i]], collapse = ","), ")", 
        sep = "")
